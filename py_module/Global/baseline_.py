...
@contextmanager
def time_limit(seconds=3600 * 10):
...
def generating_model_circuit_with_specified_noise(mq_circuit, cirq_circuit, qubits_num,
                                                  noise_type_, noise_list_, kraus_file_, noise_p_, model_name_):
    cirq_qubits = cirq.GridQubit.rect(1, qubits_num)
    # symbols = iter(variables)
    mq_qubits = [i for i in range(qubits_num)]
    ...
    if noise_p_ > 1e-7:
        if noise_type_ == "mixed":
            l = len(noise_list_)
            for q in mq_qubits[::l]:
                for i in range(l):
                    if q + i < qubits_num:
                        cirq_circuit += noise_op_cirq[noise_list_[i]](noise_p_)(cirq_qubits[q + i])
            ...
            cirq_circuit += noise_op_cirq[noise_type_](noise_p_).on_each(*cirq_qubits)
            for q in mq_qubits:
    ...
    mq_circuit += Measure('q{}'.format(mq_qubits[-1])).on(mq_qubits[-1])
...
def calculate_lipschitz(cirq_circuit: cirq.Circuit, cirq_qubits):
    ...
    # qubits = cirq.GridQubit.rect(1, len(cirq_circuit.all_qubits()))
...
def circuit2M(circuit, noise_type, noise_list, kraus_file, p):
...
def batch_evaluate(model):
    ...
    with (open("./results/baseline_comparison.csv", "a+") as csvfile):
        ...
        for i in range(len(args)):
            noisy_p, epsilon, delta = args[i][0], args[i][1], args[i][2]
            ...
            # specified noise
            # mq_circ = mindquantum.Circuit(random_mq_circuit)
            # cirq_circ = cirq.Circuit(random_cirq_circuit)
            mq_circ = mindquantum.Circuit(origin_mq_circuit)
            cirq_circ = cirq.Circuit(origin_cirq_circuit)
            if '.qasm' in model:
                final_mq_circuit, final_cirq_circuit = generating_circuit_with_specified_noise(
                    mq_circ, cirq_circ, noise_type, noise_list, kraus_file, noisy_p, model_name)
            else:
                final_mq_circuit, final_cirq_circuit = generating_model_circuit_with_specified_noise(
                    mq_circ, cirq_circ, qubits_num, noise_type, noise_list, kraus_file, noisy_p, model_name)
            ...
            t_start = time.time()
            a, _ = np.linalg.eig(circuit2M(origin_cirq_circuit, noise_type, noise_list, kraus_file, noisy_p))
            ...
            w.writerow([model_name, noise_, noisy_p, (epsilon, delta),
                        res, '%.5f' % k_base, '%.2f' % time_base, '%.5f' % final_k, '%.2f' % final_time])
