...
if arg_num <= 2:  # random noise
    ...
    file_name = "aci_{}_{}".format(noise_type, str(noisy_p))
    # model_path = "./saved_models/cr_{}_{}".format(noise_type, str(noisy_p))
else:  # specified noise
    RANDOM = False
    noise_type = str(sys.argv[1])
    noisy_p = float(sys.argv[arg_num - 2])
    choice = str(sys.argv[arg_num - 1])

    if noise_type == 'mixed':
        noise_list = [i for i in sys.argv[2: arg_num - 2]]
        ...
        file_name = "aci_mixed_{}_{}".format('_'.join(noise_list_), str(noisy_p))
        # model_path = "./saved_models/cr_mixed_{}_{}".format('_'.join(noise_list_), str(noisy_p))
    elif noise_type == 'custom':
        kraus_file = sys.argv[2]
        file_name = "aci_custom_{}_{}".format(kraus_file[kraus_file.rfind('/') + 1:-4], str(noisy_p))
        ...
        file_name = "aci_{}_{}".format(noise_, str(noisy_p))
    ...
    if p > 1e-5:
        if noise_type == "mixed":
            ...
            for q in range(cirq.num_qubits())[::l]:
                for i in range(l):
                    circuit += noise_op_cirq[noise_list[i]](p).on(qubits[q + i])
        elif noise_type == "custom":
            # TODO
...
def print_model_circuit(variables, p, noise_type):
    ...
    circuit.svg().to_file("./model_circuits/{}.svg".format(file_name))
...
def circuit2M(p, variables, noise_type):
    ...
    if p > 1e-5:
        ...
        if noise_type == "mixed":
            ...
            for q in range(cirq.num_qubits())[::l]:
                for i in range(l):
                    noisy_kraus += cirq.channel(noise_op_cirq[noise_list[i]](p)(q + i))
        elif noise_type == "custom":
            # TODO
            data = load(kraus_file)
            noisy_kraus = data['kraus']
            ...
            # noise = noise_op_cirq[noise_type]
            noisy_kraus = [cirq.channel(noise_op_cirq[noise_type](p)(q)) for q in qubits]
